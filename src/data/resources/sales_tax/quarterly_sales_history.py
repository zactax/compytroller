...
class QuarterlySalesHistory:
    ...
    def summary_report(self, summary_type: str = "In-State"):
        ...
        for i in ['ccmOption', 'cityName']:
            if i in self.payload:
                del self.payload[i]
        self.summary = True
    ...
    def report_by_ccma(self, ccm_option: str, jurisdiction_name: str):
        ...
        if 'summaryType' in self.payload:
            del self.payload['summaryType']
        self.summary = False
    ...
    def with_summary_type(self, summary_type: str):
        # "In-State", "Out-of-State", "Grand Totals"
        if not self.summary:
            raise RuntimeError("Can only set summary type for summary reports.")
        self.payload["summaryType"] = summary_type
        return self
    ...
    def with_industry(self, label: str):
        self.label = label
        if self.summary:
            self.payload["selectorOptionsSummary"] = self.SELECTOR_MAP.get(label)
        else:
            self.payload["selectorOptionsReportBy"] = self.SELECTOR_MAP.get(label)
        return self
    ...
    def get(self) -> List[QuarterlySalesHistoryData]:
        ...
        for tab in parser.css("div.tab-pane"):
            ...
            for row in tab.css("table.resultsTable tbody tr"):
                ...
                gross_sales = float(cols[1].replace(",", "")) if cols[1] else None
                taxable_sales = float(cols[2].replace(",", "")) if cols[2] else None
                num_outlets = int(cols[3].replace(",", "")) if cols[3] else None

                records.append(
                    QuarterlySalesHistoryData(
                        jurisdiction_name=self.payload.get("cityName", "Texas"),
